...
def normalize_image_digests(
    image_digest: str,
    accepted_image_digests: Sequence[str] | None = None,
) -> list[str]:
    candidates = [*(accepted_image_digests or ()), image_digest]
    normalized: list[str] = []
    ...
    if not normalized:
        raise ValueError("at least one accepted image digest is required")
    return normalized
...
<body>
    ...
    <section class="grid" aria-label="Verification checklist">
      <div class="panel">
        ...
        <ul class="checks">
          ...
          <li><span class="check">OK</span><span>Compare the attested image digest with the accepted measured digests on this page. During a rolling release, the old and new measured workloads are both listed.</span></li>
  ...
  --bundle pcr0-aws.txt.bundle \\
  --certificate-identity-regexp '^https://github.com/Lore-Hex/quill-cloud-proxy/' \\
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \\
  pcr0-aws.txt</pre>
      <p>That identity is what you pin. A signature alone proves authorship, not recency &mdash; to confirm you are looking at the <em>newest</em> record rather than an older one replayed at you, query the transparency log for that same identity. The log is append-only, so a newer entry cannot be hidden from you.</p>
      <p>One thing the log cannot tell you is whether a measurement is still what is <em>running</em>. A deployment unchanged for months should carry a months-old signature, so age is not drift. Only comparing a published record against a live attestation distinguishes the two, which is what our own scheduled check does &mdash; and what you can do yourself with the endpoints above.</p>
    </section>
    <section class="grid">
