...
QUILL_NEAR_AI_SECRET="${QUILL_NEAR_AI_SECRET:-trustedrouter-near-ai-api-key}"
...
# DNS-01 renewer + fallback CA (task #56: an LE outage must not be a total
# TLS outage). Cloud DNS project/zone make the DNS-01 renewer actually START
# on GCP — before this it only looked configured (no provider env, so the
# main.go gate never passed). The fallback directory is Google Trust
# Services; its EAB rides Secret Manager as "<kid>:<hmac>" under the secret
# named here (mint with `gcloud publicca external-account-keys create` —
...
# FLIPPED 2026-08-27: activate dormant #250 connection reuse. To revert to
# legacy behavior, set QUILL_KEEPALIVE=legacy and redeploy. The parser enables
# on/true/1, disables off/false/0, and safely treats any other value as legacy.
QUILL_KEEPALIVE="${QUILL_KEEPALIVE:-on}"
# Optional tee-env segment: absence preserves the enclave's default of TLS
# resumption on; set a non-empty value (notably "off") to inject the override.
TLS_RESUMPTION_TEE_ENV=""
if [ -n "${QUILL_TLS_RESUMPTION:-}" ]; then
  TLS_RESUMPTION_TEE_ENV="|tee-env-QUILL_TLS_RESUMPTION=${QUILL_TLS_RESUMPTION}"
fi
...
configure_optional_provider_secret QUILL_REGOLO_SECRET trustedrouter-regolo-api-key
configure_optional_provider_secret QUILL_REDPILL_SECRET trustedrouter-redpill-api-key
configure_optional_provider_secret QUILL_META_DIRECT_SECRET trustedrouter-meta-direct-api-key
configure_optional_provider_secret QUILL_GENERAL_COMPUTE_SECRET trustedrouter-general-compute-api-key
configure_optional_provider_secret QUILL_INFOMANIAK_SECRET trustedrouter-infomaniak-api-key
configure_optional_provider_secret QUILL_CONFIDENTIAL_AI_SECRET trustedrouter-confidential-ai-api-key
configure_optional_provider_secret QUILL_SCALEDOWN_SECRET trustedrouter-scaledown-api-key
...
configure_optional_provider_secret QUILL_FAL_SECRET trustedrouter-fal-api-key
configure_optional_provider_secret QUILL_BYTEPLUS_SECRET trustedrouter-byteplus-api-key
configure_optional_provider_secret QUILL_TENCENT_SECRET trustedrouter-tencent-tokenhub-api-key
...
configure_optional_provider_secret QUILL_VERCEL_AI_GATEWAY_SECRET trustedrouter-vercel-ai-gateway-api-key
configure_optional_provider_secret QUILL_TYPESAFE_SECRET trustedrouter-typesafe-api-key
...
AZURE_TEE_ENV=""
if [ -n "${QUILL_AZURE_SECRET}" ]; then
  AZURE_TEE_ENV="|tee-env-QUILL_AZURE_SECRET=${QUILL_AZURE_SECRET}"
fi
...
if { [ -n "${QUILL_DATABRICKS_SECRET}" ] && [ -z "${QUILL_DATABRICKS_HOST_SECRET}" ]; } ||
   { [ -z "${QUILL_DATABRICKS_SECRET}" ] && [ -n "${QUILL_DATABRICKS_HOST_SECRET}" ]; }; then
  echo "Databricks token and host secrets must be configured together" >&2
  exit 1
fi
DATABRICKS_TEE_ENV=""
