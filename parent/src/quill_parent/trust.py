...
{pcr0}</pre>
  ...
  <p>The KMS key policy that gates Bedrock access is bound to that PCR0 value;
     ...
     See
     <a href="{_link(settings.git_commit, "../quill-cloud-infra/modules/kms/main.tf")}">
     quill-cloud-infra/modules/kms/main.tf</a>.</p>

  <h2>Code</h2>
  <ul>
    <li><a href="{_link(commit, "enclave/src/quill_enclave/main.py")}">
        enclave/src/quill_enclave/main.py</a> — the entire prompt path</li>
    <li><a href="{_link(commit, "parent/src/quill_parent/relay.py")}">
        parent/src/quill_parent/relay.py</a> — the byte pump (no inspection)</li>
    ...
    <li><a href="{_link(commit, "enclave/tests/test_no_io.py")}">
        enclave/tests/test_no_io.py</a> — AST proof there is no I/O in the enclave</li>
  </ul>
