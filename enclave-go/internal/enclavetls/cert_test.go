...
func TestNewSelfSigned_ValidCert(t *testing.T) {
	srv, err := NewSelfSigned("api.quill.lorehex.co")
	if err != nil {
	...
	if leaf.Subject.CommonName != "api.quill.lorehex.co" {
		t.Errorf("CN = %q, want api.quill.lorehex.co", leaf.Subject.CommonName)
	}
	if len(leaf.DNSNames) != 1 || leaf.DNSNames[0] != "api.quill.lorehex.co" {
		t.Errorf("DNSNames = %v, want [api.quill.lorehex.co]", leaf.DNSNames)
	}
	if leaf.IsCA {
