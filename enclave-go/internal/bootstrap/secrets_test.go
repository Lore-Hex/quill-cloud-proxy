...
func TestSecretBindingsTableIsWellFormed(t *testing.T) {
	if len(secretBindings) != 105 {
		t.Errorf("secretBindings has %d entries, want 105", len(secretBindings))
	}
	providers := 0
	...
	if providers != 95 {
		t.Errorf("%d provider bindings, want 95 — the 'at least one provider' guard counts these", providers)
	}
