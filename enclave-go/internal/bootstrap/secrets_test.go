...
func TestSecretBindingsTableIsWellFormed(t *testing.T) {
	if len(secretBindings) != 82 {
		t.Errorf("secretBindings has %d entries, want 82", len(secretBindings))
	}
	providers := 0
	...
	if providers != 73 {
		t.Errorf("%d provider bindings, want 73 — the 'at least one provider' guard counts these", providers)
	}
