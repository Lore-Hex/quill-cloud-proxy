...
- **Terraform**:
  ...
  - Front it with `api.quillrouter.com` (CNAME from Cloudflare).
...
curl -s https://api.quillrouter.com/attestation | python verify-attestation.py
# 2. Confirm the live TLS handshake presents that exact cert.
echo | openssl s_client -servername api.quillrouter.com \
        -connect api.quillrouter.com:443 2>/dev/null \
