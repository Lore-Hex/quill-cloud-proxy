...
|---|---|
| `acme_get_certificate_failed sni=""` | **Mostly background noise** (random internet scanners hitting the raw instance IP with no SNI). The old HTTP/SNI-less LB health-checker used to add ~50/hour of these; that drumbeat is gone (there is no LB health-checker anymore — the LB was removed 2026-06-18, and a brief 2026-06-19 TCP:443 trial was torn down). A *sustained* spike now is more likely real (a misconfigured client, or a region whose `API_HOST` lost its canonical SNIs). Compare the count against a known-good hour with `tools/dx/enclave-logs.sh --since <healthy-hour>` before chasing it. |
